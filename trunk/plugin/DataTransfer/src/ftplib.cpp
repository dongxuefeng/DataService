...
int net_read(int x, char * y, int z)
{
	return recv(x,y,z,0);
}

int net_write(int x, char * y, int z)
{
	return send(x,y,z,0);
}
...
static int readline(char *buf,int max,netbuf *ctl)
{
    ...
    do
    {
	    ...
	    end = (char*)memccpy((void*)bp,ctl->cget,'\n',x);
	    if (end != NULL)
		x = end - bp;
	    retval += x;
	    bp += x;
...
/*
 * read a response from the server(only for MKD, if the path is exist, there is no err return) 2012.11.18 zy
 *
 * return 0 if first char doesn't match
 * return 1 if first char matches
 */
static int readresp_plus(char c, netbuf *nControl)
{
    char match[5];
    int ret;
    ...
    */
    ...
    if (nControl->response[0] == c || (nControl->response[0] == '5' && nControl->response[1] == '2' && nControl->response[2] == '1') || 
    	(nControl->response[0] == '5' && nControl->response[1] == '5' && nControl->response[2] == '0'))
	return 1;
    return 0;
}
...
static int readresp(char c, netbuf *nControl)
{
    ...
    memset(nControl->response, 0, 256);
    ret = readline(nControl->response,256,nControl);
    if (ret < 0)
    {
        /*
 ...
 */
 int FtpConnect(const char *host, netbuf **nControl, int timeout_ms)
{
    int sControl;
...
static int FtpSendCmd(const char *cmd, char expresp, netbuf *nControl)
{
    ...
    if(strncmp(buf, "MKD", 3) == 0){
    	return readresp_plus(expresp, nControl);
    }
...
static int FtpOpenPort(netbuf *nControl, netbuf **nData, int mode, int dir, int timeout_ms)
{
    ...
    {
	if (getsockname((SOCKET)nControl->handle, (struct sockaddr*)&sin.sa, (int*)&l) < 0)
	{
	    //perror("getsockname");
	    return -6;
	}
	...
	if (getsockname((SOCKET)sData, (struct sockaddr*)&sin.sa, (int*)&l) < 0)
	    return -15;
...
static int FtpAcceptConnection(netbuf *nData, netbuf *nControl)
{
	...
	{
	    l = sizeof(addr);
	    sData = accept((SOCKET)nData->handle, (struct sockaddr*)&addr, (int*)&l);
	    i = errno;
	    net_close(nData->handle);
	    if (sData > 0)
	    {
 ...
 int FtpRead(void *buf, int max, netbuf *nData)
{
	...
	if (i != 1)
	    return 0;
        */
        i = net_read((int)nData->handle, (char *)buf, (int)max);
        if(i == -1)
        {
            //network_error = 1;
        }
 ...
 int FtpWrite(void *buf, int len, netbuf *nData)
{
    ...
    {
        //socket_wait(nData);
        i = net_write((int)nData->handle, (char *)buf, len);
        //if(i == -1)
            //network_error = 1;
    }
    if (i == -1)
